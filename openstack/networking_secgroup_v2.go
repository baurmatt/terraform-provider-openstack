...
func networkingSecgroupV2RulesCheckForErrors(d *schema.ResourceData) error {
	...
	for _, rawRule := range rawRules {
		...
		remote_ip_prefix := rawRuleMap["remote_ip_prefix"].(string)
		...
		if self {
			if remote_ip_prefix != "" || remote_group_id != "" {
				return errorMessage
			}
		}
	}
